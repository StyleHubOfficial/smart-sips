      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {GENERATORS.map((gen, idx) => (
          <motion.div
            ...
            whileHover={isSmartPanelMode ? {} : { scale: 1.02 }}
            whileTap={isSmartPanelMode ? {} : { scale: 0.98 }}
          >
            <Link to={gen.link} className="block h-full">
              <div className={`glass-panel p-8 rounded-3xl border border-white/10 ${isSmartPanelMode ? '' : 'hover:border-white/30 hover:shadow-[0_20px_50px_rgba(0,0,0,0.5)]'} h-full flex flex-col group relative overflow-hidden`}>
                {!isSmartPanelMode && (
